...
public class PowerHost extends HostDynamicWorkload {
	...
	public double getEstimatedPower(double mips)
	{
		double pwr =0;
		
		double currPwr = this.getPower();
		
		double currMIPS = this.getUtilizationMips();
		
		double newMIPS = currMIPS + mips;
		
		double newUtil = (newMIPS) / this.getTotalMips();
		
		newUtil = Math.min(1, newUtil);
		
		double newPwr = this.getPower(newUtil);
